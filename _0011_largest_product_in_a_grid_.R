...
n <- 20  # number of cells in a row/collumn of the grid
k <- 4   # number of adjacent numbers taken to calculate their product

my_html <- readLines(
    
    con = "https://projecteuler.net/problem=11",
    encoding = "UTF-8"
...
for(i in 1:dim(my_grid)[1]){
    ...
    for(j in 1:dim(my_grid)[2]){
        ...
        if(
            ...
            if(
                prod(my_grid[i, j:(j + k - 1)]) > my_output
            ){
                my_output <- prod(my_grid[i, j:(j + k - 1)])
            }
        }
        ...
        if(
            ...
            if(
                prod(my_grid[i:(i + k - 1), j]) > my_output
            ){
                my_output <- prod(my_grid[i:(i + k - 1), j])
            }
        }
